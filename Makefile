...
# Directory containing the Bluespec Trddsraining distribution directory
# DISTRO ?= ..
...
BSIM_DIRS = -simdir $(BUILD_DIR)/sim -bdir $(BUILD_DIR)/bin -info-dir $(BUILD_DIR)/info
BSIM_EXE = $(BUILD_DIR)/exec/$(TOPMODULE)_bsim.o
...
build_bsim:
	mkdir  $(BUILD_DIR)
	mkdir  $(BUILD_DIR)/exec
	mkdir  $(BUILD_DIR)/sim
	mkdir  $(BUILD_DIR)/bin
	mkdir  $(BUILD_DIR)/info
	mkdir  $(BUILD_DIR)/dump


.PHONY: compile
compile: build_bsim
	@echo Compiling for Bluesim ...
#   bsc -u -sim -show-schedule $(BSIM_DIRS) $(BSC_COMP_FLAGS) $(BSC_PATHS) -g $(TOPMODULE)  $(TOPFILE)
	bsc -u -sim -show-schedule $(BSIM_DIRS) $(BSC_COMP_FLAGS) $(BSC_PATHS) -g $(TOPMODULE)  $(TOPFILE)
...
link:
	...
	bsc -e $(TOPMODULE) -sim -o $(BSIM_EXE) $(BSIM_DIRS) $(BSC_LINK_FLAGS) $(BSC_PATHS) # \
		# $(DISTRO)/Common/C_imports.c
	@echo Linking for Bluesim finished

.PHONY: simulate
simulate:
	@echo Bluesim simulation ...
	./$(BSIM_EXE) -V $(BUILD_DIR)/dump/dump.vcd
